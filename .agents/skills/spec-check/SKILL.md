...
| ❌ Không được | Lý do |
|---|---|
| Tạo, sửa, di chuyển hoặc xóa file | Nghiệm thu là chấm, không phải sửa. Tick `- [x]` trong checklist và cập nhật `ai/current.md` **là việc của user** |
...
Chỉ thực hiện thao tác đọc file, liệt kê file và tìm kiếm nội dung; ưu tiên công cụ tìm kiếm nhanh như `rg` nếu môi trường hỗ trợ. Báo cáo viết **tiếng Việt**, khớp giọng docs của dự án.
