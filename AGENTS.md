...
## Tương thích AI agent
- Workflow dùng chung có nguồn chính ở `.agents/skills/`; sửa skill tại đó.
- Thư mục riêng của từng agent (`.claude/`, `.codex/`, `.cursor/`…) chỉ chứa adapter hoặc cấu hình riêng, không giữ bản sao rule nghiệp vụ hay workflow.
